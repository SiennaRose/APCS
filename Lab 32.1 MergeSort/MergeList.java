...
public class MergeList
{
  ...
  {
    int halfway = 0; 
    halfway = listA.size()/2;
    ListIterator listIter = listA.listIterator(halfway); //finds halfway point
    
    while(listIter.hasNext())
    {
        listB.add(listA.remove(halfway)); //removes node from list on the second half and adds to the new listB
    }
  ...
  public LinkedList merge(LinkedList listA, LinkedList listB)
  {
    ...
    ListIterator iterA = listA.listIterator(); //iter to traverse listA
    ListIterator iterB = listB.listIterator(); //iter to traverse listB
    
    while(iterA.hasNext() || iterB.hasNext()) //while either list still has nodes
    {
        if(!iterA.hasNext()) //if listA doesn't have anymore nodes
        {
                mergedList.add(listB.remove()); //remove the remaining nodes in listB and add to the combined and sorted list
        }
        else
            if(!iterB.hasNext()) //if listB doesn't have anymore nodes
            {
                    mergedList.add(listA.remove()); //remove the remaining nodes in listA and add to the combined and sorted list
            }
            else
                if(((Item)listA.getFirst()).getId() < ((Item)listB.getFirst()).getId()) //if the first value in listB is less than the first value in listB then 
                {
                    mergedList.add(listA.remove());
                }
                else
                {
                    mergedList.add(listB.remove());
                }
    }           
